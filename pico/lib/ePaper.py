# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#
# Original code from: https://github.com/phoreglad/pico-epaper
# Updated code for Pico 2W: https://gist.github.com/phoreglad/04986ba4a3dff34157c4cb3dc6f1dd80
# Author: phoreglad, https://github.com/phoreglad
#
...
class EinkPIO(EinkBase):
    ...
    def __init__(self, sm_num=0, dma=5, *args, **kwargs):
        ...
        self._sm_shiftctrl = (0x502000d0 + 0x100000 * (self._sm_num // 4)
                              + 0x18 * (self._sm_num % 4))
        self._dreq = self._sm_num % 4 + 8 * (self._sm_num // 4)
        self._pio_setup()
    ...
    def _dma_start(self, buffer):
        d = self.DMA()
        c = d.pack_ctrl(size=0, treq_sel=self._dreq, inc_write=False)
        d.config(
            read=self.addressof(buffer),
            write=self._sm,
            count=len(buffer),
            ctrl=c,
            trigger=True
    ...
    def _send_buffer(self, buffer):
        ...
        dma = self._dma_start(buffer)

        start = ticks_ms()
        while dma.active() and ticks_diff(ticks_ms(), start) < 5000:
            pass
        self._cs(1)
        dma.close()
...
if __name__ == "__main__":
    ...
    epd = EinkPIO(sm_num=0, rotation=270, use_partial_buffer=True)
    epd.fill()
